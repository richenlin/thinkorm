...
export default class extends base {
    ...
    parseWhereItem(key, val) {
        ...
        // where({id: [1, 2, 3]})
            ...
            let flag = ORM.isNumber(val[0]) || ORM.isNumberString(val[0]);
            if (flag) {
                flag = val.every(item => {
    ...
    parseThinkWhere(key, val) {
        switch (key) {
            ...
            case '_query':
                let where = ORM.isString(val) ? querystring.parse(val) : val;
                let logic = this.getLogic(where);
                let arr = [];
    ...
    parseUnion(union) {
        ...
        if (ORM.isArray(union)) {
            ...
            union.forEach(item => {
                ...
                sql += '(' + (ORM.isObject(item.union) ? this.buildSelectSql(item.union) : item.union) + ')';
            });
            return sql;
        } else {
    ...
    parseSql(sql, options) {
        ...
        return sql.replace(/\%([A-Z]+)\%/g, (a, type) => {
            ...
            if (ORM.isFunction(this['parse' + ucfirst])) {
