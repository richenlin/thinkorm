...
module.exports = class Model extends liteq {
    /**
     * Creates an instance of ThinkORM.
     * @param {any} args 
     */
    constructor(...args) {
        super(...args);
    ...
    async add(data, options) {
        try {
            ...
            let _data = await this._beforeAdd(data, options);
            _data = valid.validData(this.fields, data, this.validations, 'ADD');
            if (helper.isEmpty(_data)) {
                throw Error('Data can not be empty');
            }
            let result = await super.add(_data, options);
            await this._afterAdd(result, options);
            return result;
        } catch (e) {
            return this.error(e);
        }
    ...
    async update(data, options) {
        try {
            let _data = await this._beforeUpdate(_data, options);
            _data = valid.validData(this.fields, _data, this.validations, 'UPDATE');
            if (helper.isEmpty(_data)) {
                throw Error('Data can not be empty');
            }
    ...
    async migrate() {
        ...
        if (!this.instance.migrate) {
            return this.error('Adapter is not support migrate');
        }
        try {
            let result = await instance.migrate({
