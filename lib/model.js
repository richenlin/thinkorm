    ...
    _class.prototype.__checkData = function __checkData(data, options) {
        ...
        try {
                ...
                if (this.fields[field]) {
                    if (this.fields[field].type) {
                        ...
                        switch (this.fields[field].type) {
                            case 'integer':
                            case 'float':
                                if (!_lib2.default.isNumber(data[field])) {
                                    return this.error(field + '值类型错误');
                                }
                                break;
                            case 'json':
                                if (!_lib2.default.isJSONObj(data[field])) {
                                    return this.error(field + '值类型错误');
                                }
                                break;
                            case 'array':
                                if (!_lib2.default.isArray(data[field])) {
                                    return this.error(field + '值类型错误');
                                }
                                break;
                            case 'string':
                            case 'text':
                                if (!_lib2.default.isString(data[field])) {
                                    return this.error(field + '值类型错误');
                                }
                                break;
                            default:
            ...
            for (var _field2 in this.fields) {
                if (method === 'ADD') {
                    //新增数据add
                    _lib2.default.isEmpty(data[_field2]) && !_lib2.default.isEmpty(this.fields[_field2].defaultsTo) && (data[_field2] = this.fields[_field2].defaultsTo);
                } else if (method === 'UPDATE') {
                    //编辑数据update
                    data.hasOwnProperty(_field2) && _lib2.default.isEmpty(data[_field2]) && !_lib2.default.isEmpty(this.fields[_field2].defaultsTo) && (data[_field2] = this.fields[_field2].defaultsTo);
                }
                //处理特殊类型字段
                if (adpCase[this.config.db_type] && data[_field2] && this.fields[_field2] && typeCase[this.fields[_field2].type]) {
