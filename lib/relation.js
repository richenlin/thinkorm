...
module.exports = class relation {
                    ...
                    if (rtype && rtype in caseList) {
                        if (lib.isArray(data)) {
                            for (let [k, v] of data.entries()) {
                                ps.push(caseList[rtype](config, options, rels[n], data[k]).then(function (res) {
                            ...
                            data[rels[n].name] = yield caseList[rtype](config, options, rels[n], data);
                        }
                    }
                }
            }
     ...
     * @returns 
     */
    static postRelationData(adapter, config, options, result, data, postType) {
        return (0, _asyncToGenerator3.default)(function* () {
            let caseList = {
