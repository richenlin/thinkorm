...
module.exports = class relation {
    ...
    static setRelations(name, config) {
        ...
        console.log(__thinkorm.collections[name].relationShip);
     ...
     * @returns 
     */
    static getRelationData(adapter, config, options, data) {
                    ...
                    if (rtype && rtype in caseList) {
                        if (lib.isArray(data)) {
                            for (let [k, v] of data.entries()) {
                                ps.push(caseList[rtype](config, rels[n], data[k]).then(function (res) {
                            ...
                            data[rels[n].name] = yield caseList[rtype](config, rels[n], data);
                        }
                    }
                }
            }
     ...
     * @param {any} postType 
     */
    static postRelationData(adapter, result, config, options, data, postType) {
        return (0, _asyncToGenerator3.default)(function* () {
            let caseList = {
