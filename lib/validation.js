...
const validData = function (fields, data, vaildRules, method = '') {
    ...
    for (let field in fields) {
        if (method === 'ADD') { //新增数据add
            if (lib.isEmpty(data[field]) && (fields[field].defaults !== undefined && fields[field].defaults !== null)) {
                data[field] = fields[field].defaults;
            }
            //非主键字段就检查
            dataCheckFlag = !fields[field].pk ? true : false;
            //定义了规则就检查
            ruleCheckFlag = vaildRules[field] ? true : false;
        } else if (method === 'UPDATE') { //编辑数据update
            if (data[field] !== undefined && lib.isEmpty(data[field]) && (fields[field].defaults !== undefined && fields[field].defaults !== null)) {
                data[field] = fields[field].defaults;
            }
            //更新包含字段就检查,主键除外(因为主键不会被更新)
            dataCheckFlag = (data[field] !== undefined && !fields[field].pk) ? true : false;
            //更新包含字段且定义了规则就检查
