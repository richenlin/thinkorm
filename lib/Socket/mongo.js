...
var _class = function (_base) {
        ...
        return this.connect().then(function (conn) {
            ...
            switch (options.method) {
                ...
                case 'SELECT':
                    ...
                    break;
                case 'COUNT':
                    sql = '' + sql + (options.where ? '.find(' + (0, _stringify2.default)(options.where) + ')' : '.find()') + '.count()';
            ...
            _this3.config.logSql && ORM.log(sql, 'SQL', startTime);
            return rows;
        }).catch(function (err) {
            return _promise2.default.reject(err);
        });
    ...
    _class.prototype.execute = function execute(options, data) {
        ...
        return this.connect().then(function (conn) {
            ...
            _this4.config.logSql && ORM.log(sql, 'SQL', startTime);
            return rows;
        }).catch(function (err) {
            return _promise2.default.reject(err);
        });
