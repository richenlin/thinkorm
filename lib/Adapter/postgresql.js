...
exports.default = class extends _base2.default {
    ...
    connect() {
        ...
        if (this.config.db_ext_config.read_write && _lib2.default.isArray(this.config.db_host)) {
            ...
            return _promise2.default.all([new _postgresql2.default(configMaster).connect(), new _postgresql2.default(configSlave).connect()]).then(cons => {
                this.handel = { RW: true };
    ...
    query(sql) {
        ...
        return this.connect().then(conn => {
            ...
            this.close();
            this.logSql && _lib2.default.log(sql, 'PostgreSQL', startTime);
    ...
    native(tableName, sqlStr) {
        ...
        return this.connect().then(conn => {
            ...
            this.close();
            this.logSql && _lib2.default.log(ouputs.sql, 'PostgreSQL', startTime);
            return _promise2.default.reject(err);
        }).then((res = {}) => {
            connection.release && connection.release();
            this.logSql && _lib2.default.log(ouputs.sql, 'PostgreSQL', startTime);
