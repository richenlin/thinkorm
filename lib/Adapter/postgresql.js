...
const socket = require('../Socket/postgresql');
...
module.exports = class extends base {
    ...
    connect() {
        ...
        if (this.config.db_ext_config.read_write && lib.isArray(this.config.db_host)) {
            ...
            return _promise2.default.all([new socket(configMaster).connect(), new socket(configSlave).connect()]).then(cons => {
                this.handel = { RW: true };
    ...
    query(sql) {
        ...
        return this.connect().then(conn => {
            ...
            this.close();
            lib.logs(sql, 'PostgreSQL', startTime, this.logSql);
    ...
    native(tableName, sqlStr) {
        ...
        return this.connect().then(conn => {
            ...
            this.close();
            lib.logs(ouputs.sql, 'PostgreSQL', startTime, this.logSql);
            return _promise2.default.reject(err);
        }).then((res = {}) => {
            connection.release && connection.release();
            lib.logs(ouputs.sql, 'PostgreSQL', startTime, this.logSql);
