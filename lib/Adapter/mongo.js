...
var _class = function (_base) {
    ...
    _class.prototype.sum = function sum(field) {
        ...
        return this.connect().then(function (conn) {
            ...
            if (_lib2.default.isEmpty(res.options.group)) {
                ...
                _this8.sql = 'db.' + res.options.table + '.aggregate(' + (0, _stringify2.default)(pipe) + ')';
                handler = fn(pipe);
            } else {
                res.options.group.initial = {
                    "sumid": 0
                ...
                res.options.group.cond = res.options.where;
                _this8.sql = 'db.' + res.options.table + '.group(' + (0, _stringify2.default)(res.options.group) + ')';
                handler = collection.group(res.options.group);
            }
    ...
    _class.prototype.find = function find() {
        ...
        return this.connect().then(function (conn) {
            ...
            if (_lib2.default.isEmpty(res.options.group)) {
                _this9.sql = 'db.' + res.options.table + (res.options.where ? '.findOne(' + (0, _stringify2.default)(res.options.where) + ')' : '.findOne()');
                handler = collection.findOne(res.options.where || {});
            } else {
                res.options.group.cond = res.options.where;
                _this9.sql = 'db.' + res.options.table + '.group(' + res.options.group.key + ',' + res.options.group.cond + ',' + res.options.group.initial + ',' + res.options.group.reduce + ')';
                //handler = collection.group(res.options.group);
    ...
    _class.prototype.select = function select() {
        ...
        return this.connect().then(function (conn) {
            ...
            if (_lib2.default.isEmpty(res.options.group)) {
                _this10.sql = '' + _this10.sql + (res.options.where ? '.find(' + (0, _stringify2.default)(res.options.where) + ')' : '.find()');
                handler = collection.find(res.options.where || {});
            } else {
                res.options.group.cond = res.options.where;
                _this10.sql = _this10.sql + '.group(' + res.options.group.key + ',' + res.options.group.cond + ',' + res.options.group.initial + ',' + res.options.group.reduce + ')';
                //handler = collection.group(res.options.group);
                handler = collection.group(res.options.group.key, res.options.group.cond, res.options.group.initial, res.options.group.reduce);
            }
            return _this10.query(handler.toArray(), startTime);
        });
    };
