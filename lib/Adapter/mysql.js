...
exports.default = class extends _base2.default {
    ...
    connect() {
        ...
        if (this.config.db_ext_config.read_write && _lib2.default.isArray(this.config.db_host)) {
            ...
            return _promise2.default.all([new _mysql2.default(configMaster).connect(), new _mysql2.default(configSlave).connect()]).then(cons => {
                this.handel = { RW: true };
    ...
    migrate(schema, config) {
        ...
        let tableName = `${config.db_prefix}${_lib2.default.parseName(schema.name)}`;
    ...
    query(sql) {
        ...
        return this.connect().then(conn => {
            ...
            let fn = _lib2.default.promisify(connection.query, connection);
            return fn(sql);
        }).catch(err => {
            this.close();
            _lib2.default.log(sql, 'MySQL', startTime, this.logSql);
    ...
    native(tableName, sqlStr) {
        ...
        return this.connect().then(conn => {
            ...
            this.close();
            _lib2.default.log(ouputs.sql, 'MySQL', startTime, this.logSql);
            return _promise2.default.reject(err);
        }).then((rows = []) => {
            connection.release && connection.release();
            _lib2.default.log(ouputs.sql, 'MySQL', startTime, this.logSql);
        ...
        return (0, _asyncToGenerator3.default)(function* () {
            ...
            switch (postType) {
                case 'ADD':
                    ...
                    //更新主表关联字段
                    fkey && (yield primaryModel.update({ [rel.fkey]: fkey }, { where: { [rel.primaryPk]: result } }));
                    break;
                case 'UPDATE':
                    let condition = {},
                        keys = [];
                    ...
                    //限制只能更新关联数据
                    let info = yield primaryModel.field([rel.primaryPk]).select(options).catch(function (e) {
                        return [];
                    });
                    ...
                    if (keys.length > 0) {
                        ...
                        yield model.update(relationData, { where: condition });
                    }
                    break;
                default:
                    break;
                ...
                case 'UPDATE':
                    ...
                    if (keys.length > 0) {
                        ...
                        for (let [k, v] of relationData.entries()) {
                            ...
                            yield model.update(v, { where: condition });
                        }
                    }
                    break;
                default:
        ...
        return (0, _asyncToGenerator3.default)(function* () {
            ...
            switch (postType) {
                ...
                case 'UPDATE':
                    ...
                    let primaryModel = new ORM.collections[rel.primaryName](config);
                    //限制只能更新关联数据
                    let info = yield primaryModel.join([{
