        ...
        switch (identifiers[idt]) {
            ...
            case 'NOTIN':
                if (_lib2.default.isObject(options[op])) {
                    ...
                    parseNotIn(knex, (_parseNotIn = {}, _parseNotIn[extkey] = options[op], _parseNotIn), _alias);
                }
                break;
            case 'NOT':
                if (_lib2.default.isObject(options[op])) {
                    ...
                    parseNot(knex, (_parseNot = {}, _parseNot[extkey] = options[op], _parseNot), _alias);
                }
                break;
            case 'OPERATOR':
                if (extkey !== undefined) {
                    ...
                    for (var _n in options[op]) {
                        ...
                        parseKnexWhere(knex, (_parseKnexWhere = {}, _parseKnexWhere[_n] = options[op][_n], _parseKnexWhere), _alias, op);
                    }
                }
                break;
            case 'AND':
            default:
                if (_lib2.default.isArray(options[op])) {
                    ...
                    for (var _n2 in options[op]) {
                        ...
                        parseKnexWhere(knex, (_parseKnexWhere2 = {}, _parseKnexWhere2[_n2] = options[op][_n2], _parseKnexWhere2), _alias, op);
                    }
                } else {
                    var _key = op.indexOf('.') === -1 ? alias + '.' + op : op;
...
function parseIn(knex, key, value, alias) {
    var _key = key.indexOf('.') === -1 ? alias + '.' + key : key;
